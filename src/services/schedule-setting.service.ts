...
export class ScheduleSettingService {
    ...
    async create(createScheduleSettingDto: CreateScheduleSettingDto): Promise<ScheduleSetting> {
        const existingSetting = await this.scheduleSettingRepository.findOne({
            where: { day_of_week: createScheduleSettingDto.day_of_week }
        });

        if (existingSetting) {
            throw new BadRequestException(`Schedule setting for day ${createScheduleSettingDto.day_of_week} already exists`);
        }

        const setting = this.scheduleSettingRepository.create(createScheduleSettingDto);
    ...
    async findByDay(dayOfWeek: number): Promise<ScheduleSetting> {
        ...
        if (!setting) {
            throw new NotFoundException(`Schedule setting for day ${dayOfWeek} not found`);
        }
        return setting;
    }

    async update(id: number, updateScheduleSettingDto: UpdateScheduleSettingDto): Promise<ScheduleSetting> {
        const setting = await this.findOne(id);
        
        if (updateScheduleSettingDto.day_of_week && updateScheduleSettingDto.day_of_week !== setting.day_of_week) {
            const existingSetting = await this.scheduleSettingRepository.findOne({
                where: { day_of_week: updateScheduleSettingDto.day_of_week }
            });

            if (existingSetting) {
                throw new BadRequestException(`Schedule setting for day ${updateScheduleSettingDto.day_of_week} already exists`);
            }
        }
